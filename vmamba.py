...
class SS2D(nn.Module):
    ...
    def forward_single_backward(self, x: torch.Tensor):
        ...
        y = self.out_norm(y)
        return y
    def forward(self, x: torch.Tensor, x2:torch.Tensor,**kwargs):
        #x,(b,dhw,c)
        xz = self.in_proj(x)
        x2z = self.in_proj2(x2)
        #print(xz.size())
        #print(1)

        x, z = xz.chunk(2, dim=-1)
        x2, z2 = x2z.chunk(2, dim=-1)

        channel_weight = self.senet(z,z2) #b,1,d
        #x,y,z,z2,(b,3d,c)
        #z = z.permute(0,2,1)
        ...
        y2 = self.forward_single_backward(x2)
        y = y + y2
        y = y * channel_weight
        #print(y.size())
        # if self.softmax_version:
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor,input2:torch.Tensor):
        #input2= input
        #print(input2.size(),111)
        ...
        x = input+input2 + self.drop_path(self.op(self.norm(input),self.norm(input2)))
        x = x.permute(0,2,1)
        #print(x.size())
        #if self.mlp_branch:
...
# model = VSSBlock().cuda()
# a = torch.rand(1,32,28,32,32).cuda()
# print(model(a).size())
...
class Backbone_VSSM(VSSM):
    def __init__(self, patch_size=4, in_chans=48, num_classes=2,
    ...
    def forward(self, x):
        ...
        return outs
#
# model = Backbone_VSSM().cuda()
# a = torch.rand(1,48,256,256).cuda()
# print(model(a).size())
