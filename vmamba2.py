...
class senet(nn.Module):
    def __init__(self,c=32,r=4):
        ...
        self.fc = nn.Sequential(nn.Conv1d(c,c//r,1,1,0,bias=True),nn.SiLU(),nn.Conv1d(c//r,c,1,1,0,bias=True))
        self.sigmoid = nn.Sigmoid()
        self.init_weights()
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # channle
        # x proj; dt proj =======================
        self.x_proj_forward = nn.Linear(2*self.d_model, (self.dt_rank + self.d_state * 2), bias=False)
        self.dt_proj_forward = self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, **factory_kwargs)

        # A, D =======================================
        self.A_log_forward = self.A_log_init(self.d_state, self.d_inner, merge=True)  # (K * D, N)
        self.D_forward = self.D_init(self.d_inner, merge=True)  # (K * D)

        # x proj; dt proj =======================
        self.x_proj_backward = nn.Linear(2*self.d_model, (self.dt_rank + self.d_state * 2), bias=False)
        self.dt_proj_backward = self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max,
                                             dt_init_floor, **factory_kwargs)

        # A, D =======================================
        self.A_log_backward = self.A_log_init(self.d_state, self.d_inner, merge=True)  # (K * D, N)
        self.D_backward = self.D_init(self.d_inner, merge=True)  # (K * D)
        # out proj =======================================
    ...
    def forward_single_backward(self, x: torch.Tensor):
        ...
        B, L, d = x.shape
        x = x.permute(0, 2, 1)
        x_dbl = self.x_proj_backward(rearrange(x, "b d l -> (b l) d"))  # (bl d)
        dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        dt = self.dt_proj_backward.weight @ dt.t()
        dt = rearrange(dt, "d (b l) -> b d l", l=L)
        A = -torch.exp(self.A_log_backward.float())  # (k * d, d_state)
        B = rearrange(B, "(b l) dstate -> b dstate l", l=L).contiguous()
        C = rearrange(C, "(b l) dstate -> b dstate l", l=L).contiguous()

        y = selective_scan(
            x, dt,
            A, B, C, self.D_backward.float(),
    ...
    def forward(self, x: torch.Tensor,**kwargs):
        ...
        x = self.act(self.conv2d(x)) # (b, c,dhw)
        #x2 = self.act(self.conv2d2(x2))  # (b, c,dhw)
        x = x.permute(0, 2, 1) # (b, dhw,c)
        #x2 = x2.permute(0, 2, 1)  # (b, dhw,c)
        #print(x.size())
        y = self.forward_single_forward(x)
...
class VSSBlock2(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.reduce1 = nn.Linear(4*hidden_dim,hidden_dim)
        self.reduce2 = nn.Linear(4*hidden_dim, hidden_dim)
        self.reduce3 = nn.Linear(4*hidden_dim, hidden_dim)
        self.reduce4 = nn.Linear(4*hidden_dim, hidden_dim)

        self.reduce1_ = nn.Linear(hidden_dim, hidden_dim)
        self.reduce2_ = nn.Linear(hidden_dim, hidden_dim)
        self.reduce3_ = nn.Linear(hidden_dim, hidden_dim)
        self.reduce4_ = nn.Linear(hidden_dim, hidden_dim)
    ...
    def create_combined_vector(self, a, b, c,d):
        """
        Create a combined vector according to the specified rules using PyTorch tensors.

        Args:
        a (torch.Tensor): Data from frame a.
        b (torch.Tensor): Data from frame b.
        c (torch.Tensor): Data from frame c.

        Returns:
        torch.Tensor: Combined vector.
        """
        # Check if the input tensors have the same shape
        assert a.shape == b.shape == c.shape == d.shape, "Input tensors must have the same shape"

        B, N, D = a.shape
        x = torch.zeros((B, 4 * N, D), dtype=a.dtype, device=a.device)
        indices_a = torch.arange(0, x.size(1), 4, device=x.device)
        indices_b = torch.arange(1, x.size(1), 4, device=x.device)
        indices_c = torch.arange(2, x.size(1), 4, device=x.device)
        indices_d = torch.arange(3, x.size(1), 4, device=x.device)
        # Assign data from frame a, b, and c to the combined vector according to the specified rule
        x[:, indices_a] = a
        x[:, indices_b] = b
        x[:, indices_c] = c
        x[:, indices_d] = d
    ...
    def create_inverse_vector(self, x):
        indices_a = torch.arange(0, x.size(1), 4, device=x.device)
        indices_b = torch.arange(1, x.size(1), 4, device=x.device)
        indices_c = torch.arange(2, x.size(1), 4, device=x.device)
        indices_d = torch.arange(3, x.size(1), 4, device=x.device)
        ...
        c = x[:, indices_c]
        d = x[:, indices_d]
        #reversed_vector = torch.cat([a, b, c,d], dim=1)
        return a,b,c,d#reversed_vector

    def ssm_spatial(self, x):
        a = x[:, :self.patch_num]
        b = x[:, self.patch_num:2 * self.patch_num]
        c = x[:, 2 * self.patch_num:]
        combined_vector = self.create_combined_vector(a, b, c)
        y = self.SSM_s(combined_vector)
        return self.create_inverse_vector(y)

    def forward(self, input1: torch.Tensor, input2: torch.Tensor, input3: torch.Tensor, input4: torch.Tensor):
        b, c, d, h, w = input1.size()
        num_patch = d * h * w
        input1 = input1.flatten(2).permute(0,2,1)
        input2 = input2.flatten(2).permute(0,2,1)
        input3 = input3.flatten(2).permute(0,2,1)
        input4 = input4.flatten(2).permute(0,2,1)#b,l,c

        #input_fuse = self.create_combined_vector(input1,input2,input3,input4)

        x1 = torch.cat([input1, input2, input3, input4], dim=2).permute(0, 2, 1)  # b,4C,d
        x2 = torch.cat([input4, input1, input2, input3], dim=2).permute(0, 2, 1)
        x3 = torch.cat([input3, input4, input1, input2], dim=2).permute(0, 2, 1)
        x4 = torch.cat([input2, input3, input4, input1], dim=2).permute(0, 2, 1)

        # x1 = self.reduce1(x1).permute(0, 2, 1)#b,c,l
        # x2 = self.reduce2(x2).permute(0, 2, 1)
        # x3 = self.reduce3(x3).permute(0, 2, 1)
        # x4 = self.reduce4(x4).permute(0, 2, 1)

        x1 = x1 + self.drop_path(self.op1(self.norm(x1)))
        #output1,output2,output3,output4 = self.create_inverse_vector(hyper)
        x2 = x2 + self.drop_path(self.op2(self.norm(x2)))
        x3 = x3 + self.drop_path(self.op3(self.norm(x3)))
        x4 = x4 + self.drop_path(self.op4(self.norm(x4)))



        #print(x2[:, num_patch:2 * num_patch].size())
        input_1_fusion = (
                    x1[:, :num_patch] + x2[:, num_patch:2 * num_patch] + x3[:, 2 * num_patch:3 * num_patch] + x4[:,
                                                                                                              3* num_patch:4 * num_patch]).permute(
            0, 2, 1)
        input_2_fusion = (x1[:, num_patch:2 * num_patch] + x2[:, 2 * num_patch:3 * num_patch] + x3[:,
                                                                                                3 * num_patch:4 * num_patch] + x4[
                                                                                                                               :,
                                                                                                                               :num_patch]).permute(
            0, 2, 1)
        input_3_fusion = (
                    x1[:, 2 * num_patch:3 * num_patch] + x2[:, 3 * num_patch:4 * num_patch] + x3[:, :num_patch] + x4[:,
                                                                                                                  num_patch:2 * num_patch]).permute(
            0, 2, 1)
        input_4_fusion = (
                    x1[:, 3 * num_patch:4 * num_patch] + x2[:, :num_patch] + x3[:, num_patch:2 * num_patch] + x4[:,
                                                                                                              2 * num_patch:3 * num_patch]).permute(
            0, 2, 1)

        # input_1_fusion = (
        #             x1[:, :num_patch//4] + x2[:, num_patch//4:2 * num_patch//4] + x3[:, 2 * num_patch//4:3 * num_patch//4] + x4[:,
        #                                                                                                       3* num_patch//4:4 * num_patch//4]).permute(
        #     0, 2, 1)
        # input_2_fusion = (x1[:, num_patch//4:2 * num_patch//4] + x2[:, 2 * num_patch//4:3 * num_patch//4] + x3[:,
        #                                                                                         3 * num_patch//4:4 * num_patch//4] + x4[
        #                                                                                                                        :,
        #                                                                                                                        :num_patch//4]).permute(
        #     0, 2, 1)
        # input_3_fusion = (
        #             x1[:, 2 * num_patch//4:3 * num_patch//4] + x2[:, 3 * num_patch//4:4 * num_patch//4] + x3[:, :num_patch//4] + x4[:,
        #                                                                                                           num_patch//4:2 * num_patch//4]).permute(
        #     0, 2, 1)
        # input_4_fusion = (
        #             x1[:, 3 * num_patch//4:4 * num_patch//4] + x2[:, :num_patch//4] + x3[:, num_patch//4:2 * num_patch//4] + x4[:,
        #                                                                                                       2 * num_patch//4:3 * num_patch//4]).permute(
        #     0, 2, 1)

        # input_1_fusion = (x1+x2+x3+x4).permute(0,2,1)
        # input_2_fusion = (x1 + x2 + x3 + x4).permute(0, 2, 1)
        # input_3_fusion = (x1 + x2 + x3 + x4).permute(0, 2, 1)
        # input_4_fusion = (x1 + x2 + x3 + x4).permute(0, 2, 1)

        #print(input_4_fusion.size())
        # x1 = x1.permute(0, 2, 1)  # b,l,c
        # x2 = x2.permute(0, 2, 1)
        # x3 = x3.permute(0, 2, 1)
        # x4 = x4.permute(0, 2, 1)

        # input_1_fusion = self.reduce1_(input_1_fusion)#.permute(0, 2, 1)
        # #print(x1.size())
        # input_2_fusion = self.reduce2_(input_2_fusion)#.permute(0, 2, 1)  # b,4c,l
        # input_3_fusion = self.reduce3_(input_3_fusion)#.permute(0, 2, 1)
        # input_4_fusion = self.reduce4_(input_4_fusion)#.permute(0, 2, 1)
        # input_1_fusion = torch.cat([
        #         x1[:, :num_patch//4], x2[:, num_patch//4:2 * num_patch//4] , x3[:, 2 * num_patch//4:3 * num_patch//4] , x4[:,
        #                                                                                                   3 * num_patch//4:4 * num_patch//4],dim=1).permute(
        #     0, 2, 1)
        # input_2_fusion = (x1[:, num_patch:2 * num_patch] + x2[:, 2 * num_patch:3 * num_patch] + x3[:,
        #                                                                                         3 * num_patch:4 * num_patch] + x4[
        #                                                                                                                        :,
        ...
        #
        # input_1_fusion = self.reduce1(input_1_fusion)
        # input_2_fusion = self.reduce2(input_2_fusion)
        # input_3_fusion = self.reduce3(input_3_fusion)
        # input_4_fusion = self.reduce4(input_4_fusion)

        output1 =  input_1_fusion#input1 +
        output2 =  input_2_fusion#input2 +
        output3 =  input_3_fusion#input3 +
        output4 =  input_4_fusion#input4 +
        ...
        # x3 = x3.permute(0, 2, 1)
        # x4 = x4.permute(0, 2, 1)
        # output1 = self.reduce1(x1).permute(0,2,1)
        # output2 = self.reduce2(x2).permute(0,2,1)
        # output3 = self.reduce3(x3).permute(0,2,1)
        # output4 = self.reduce4(x4).permute(0,2,1)

        output1 = output1.permute(0,2,1).view(-1, c, d, h, w)
        output2 = output2.permute(0,2,1).view(-1, c, d, h, w)
        output3 = output3.permute(0,2,1).view(-1, c, d, h, w)
        output4 = output4.permute(0,2,1).view(-1, c, d, h, w)
        return output1, output2, output3, output4


class VSSBlock3(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 32,
            drop_path: float = 0.1,
    ...
    def forward(self, input1: torch.Tensor):
        b,  d, h, w,c = input1.size()
        num_patch = d * h * w
        x1 = input1.permute(0,4,1,2,3).flatten(2).permute(0,2,1)

        x1 = x1 + self.drop_path(self.op1(self.norm(x1)))

        #output = x1 + self.drop_path(self.mlp(self.norm2(x1))) # FFN
